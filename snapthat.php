<?php
/**
* Plugin Name: SnapThat – Website Feedback, Screenshots & Tasks
* Plugin URI: https://docs.snapthat.dev/
* Description: Adds a floating button to capture annotated screenshots and create tasks with comments directly from your site. Integrates with snapThat.dev.
* Version: 1.0.0
* Requires at least: 5.8
* Tested up to: 6.8
* Requires PHP: 7.1
* Author: AdventureKingz GmbH, David Wright
* Author URI: https://getsnapthat.com
* License: GPLv2 or later
* License URI: https://www.gnu.org/licenses/gpl-2.0.html
* Text Domain: snapthat
...
*/
    ...
    class SnapThat_WP_Plugin {
        ...
        public function sanitize_options($input) {
            ...
            $output['theme']    = in_array($output['theme'], ['light','dark'], true) ? $output['theme'] : 'light';
            $output['pos']      = in_array($output['pos'], ['left','right'], true) ? $output['pos'] : 'left';
        ...
        public function render_settings_page() {
            ...
            ?>
            <div class="wrap">
                <h1><?php _e('SnapThat Plugin Settings', 'snapthat'); ?></h1>
                <p><?php _e('A SnapThat account is required to use this plugin.', 'snapthat'); ?></p>
                ...
                <form method="post" action="options.php">
                    ...
                    <table class="form-table" role="presentation">
                        <tr>
                            <th scope="row"><label for="api"><?php _e('API URL', 'snapthat'); ?></label></th>
                            <td><input type="url" name="<?php echo $this->option_name; ?>[api]" value="<?php echo esc_attr($options['api']); ?>" class="regular-text"></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="brand"><?php _e('Brand Color', 'snapthat'); ?></label></th>
                        ...
                        <tr>
                            <th scope="row"><label for="pin"><?php _e('Pin Color', 'snapthat'); ?></label></th>
                            <td><input type="text" name="<?php echo $this->option_name; ?>[pin]" value="<?php echo esc_attr($options['pin']); ?>" class="snapthat-color-field"></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="theme"><?php _e('Theme', 'snapthat'); ?></label></th>
                            <td>
                                <select name="<?php echo $this->option_name; ?>[theme]">
                                    <option value="light" <?php selected($options['theme'], 'light'); ?>><?php _e('Light', 'snapthat'); ?></option>
                                    <option value="dark" <?php selected($options['theme'], 'dark'); ?>><?php _e('Dark', 'snapthat'); ?></option>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="pos"><?php _e('Position', 'snapthat'); ?></label></th>
                            <td>
                                <select name="<?php echo $this->option_name; ?>[pos]">
                                    <option value="left" <?php selected($options['pos'], 'left'); ?>><?php _e('Left', 'snapthat'); ?></option>
                                    <option value="right" <?php selected($options['pos'], 'right'); ?>><?php _e('Right', 'snapthat'); ?></option>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Offset X', 'snapthat'); ?></th>
                        ...
                        <tr>
                            <th scope="row"><?php _e('Offset Y', 'snapthat'); ?></th>
                        ...
                        <tr>
                            <th scope="row"><?php _e('Roles/Groups', 'snapthat'); ?></th>
                            <td>
                                <?php
                                global $wp_roles;
                                foreach ($wp_roles->roles as $role_key => $role) {
                                    ?>
                                    <label>
                                        <input type="checkbox" name="<?php echo $this->option_name; ?>[roles][]" value="<?php echo esc_attr($role_key); ?>" <?php checked(in_array($role_key, $options['roles'], true)); ?>>
                        ...
                        <tr>
                            <th scope="row"><?php _e('Show publicly', 'snapthat'); ?></th>
                            <td>
                                <label>
                                    <input type="checkbox" name="<?php echo $this->option_name; ?>[public]" value="1" <?php checked($options['public'], true); ?>>
                                    <?php _e('Activate for non-logged-in visitors as well', 'snapthat'); ?>
                                </label>
                            </td>
                        </tr>
                    </table>
